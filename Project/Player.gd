...
func _input(event):
    if event is InputEventMouseMotion:
        ...
        print_debug("mouse click")
        var raycastHit = raycast.get_collider()
        if not raycastHit == null:
            print_debug("raycastHit not null")
            print_debug(raycastHit)
            if raycastHit.is_in_group("Moveable"):
                (raycastHit as RigidBody).apply_central_impulse(-raycast.get_global_transform().basis.z * 10)
